...
def build_pie_chart(site_dropdown):
    if site_dropdown == 'ALL':
        ...
        filtered_df = spacex_df[spacex_df['Launch Site'] == site_dropdown].reset_index(drop=True)
        site_counts = filtered_df['class'].value_counts().reset_index()
        site_counts.columns = ['class', 'count']
        class_names = {1: 'Success', 0: 'Failure'}
        site_counts['class'] = site_counts['class'].map(class_names)
...
def update_scatter(site_dropdown, payload_slider):
    ...
    if site_dropdown != 'ALL':
        filtered_df = filtered_df[filtered_df['Launch Site'] == site_dropdown].reset_index(drop=True)
